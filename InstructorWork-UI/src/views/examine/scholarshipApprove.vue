  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="60%">
      <el-descriptions class="margin-top" :column="3" border>
        <template slot="title">
          <el-form ref="scholarship" :disabled="formRequired" :model="scholarship" :rules="rules" label-width="120px">
            <el-form-item label="辅导员意见" prop="counselorOpinion">
              <el-input v-model="scholarship.counselorOpinion" type="textarea" autosize />
            </el-form-item>
          </el-form>
        </template>
        <template slot="extra">
        ...
        <el-descriptions-item>
          ...
          <el-tag :type="scholarship.approveStatus | statusFilter">
            {{ scholarship.approveStatus | formatStatus }}
          </el-tag>
        </el-descriptions-item>
      </el-descriptions>
...
<script>
import { getScholarshipByCounselor, passScholarship, rejectScholarship } from '@/api/scholarship' // 奖学金请求
  ...
  methods: {
    pass() {
      this.$refs.scholarship.validate(valid => {
        if (valid) {
          this.scholarship.counselorName = this.username
    ...
    getList() {
      ...
      getScholarshipByCounselor(this.page, this.limit, this.listQuery).then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['学生学号', '学生姓名', '班级', '奖学金等级', '综测排名', '证明材料', '辅导员意见', '审核意见', '审核状态', '申请时间']
        const filterVal = ['username', 'realName', 'departId', 'level', 'ranking', 'prove', 'counselorOpinion', 'auditOpinion', 'approveStatus', 'createTime']
